...
//   (no date)        → games:all, which this service no longer writes.
//
// The bare form exists only for app versions shipped before per-day loading.
// games:all is left behind as a static snapshot with a long TTL (see
// scripts/migrate-day-shards.mjs) so those clients keep working until they
// update; once it expires this falls through to the live-fetch path below.
